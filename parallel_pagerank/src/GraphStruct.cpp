...
void graphInit(GraphStruct *graph, int num_vtxs, int num_nbors, int num_parts){
	...
	if(num_vtxs > 0){
		...
		graph->degree = (int *) malloc(sizeof(int) * num_vtxs);
		graph->status = (int *) malloc(sizeof(int) * num_vtxs);

		graph->pageRank = (double *) malloc(sizeof(double) * 2 * num_vtxs);
		graph->oldPageRank = graph->pageRank;
		graph->currPageRank = graph->pageRank + num_vtxs;
